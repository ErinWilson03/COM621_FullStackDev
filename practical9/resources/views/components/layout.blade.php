...
<body>
    <header>
        <!-- page navbar -->
        <nav class="flex justify-between py-3 px-6 border-b">
            <!-- site name -->
            ...
            <div class="flex items-center gap-4">
                @guest
                    <a role="nav" href="/login">Login</a>
                    <a role="nav" href="/register">Register</a>
                @endguest
                @auth
                    <form method="post" action="{{ route('logout') }}" class="flex gap-2 items-center">
                        @csrf
                        <button type="submit" role="nav-link">Logout</button>
